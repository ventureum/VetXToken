...
var Token = artifacts.require('VetXToken')
...
contract('VetXToken', function (accounts) {
  let token

  beforeEach(async function () {
    token = await Token.new('100', 'VetX', 18, 'VTX')
  })
  ...
  describe('should fail when paused', function () {
    ...
    it('should fail when approve', async function () {
      await token.pause()
      await token.approve(accounts[1], 10).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when increaseApproval', async function () {
      await token.pause()
      await token.increaseApproval(accounts[1], 50).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when decreaseApproval', async function () {
      await token.pause()
      await token.decreaseApproval(accounts[1], 50).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when transferFrom', async function () {
      await token.pause()
      await token.transferFrom(accounts[0], accounts[1], 50).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when transferAndCall', async function () {
      await token.pause()
      const message = await Message.new()
    ...
    it('should fail when transferFromAndCall', async function () {
      await token.pause()
      const message = await Message.new()
      ...
      await token.transferFromAndCall(
        accounts[0], message.contract.address, 100, extraData, { from: accounts[0], value: 1000 }
      ).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when approveAndCall', async function () {
      await token.pause()
      const message = await Message.new()
    ...
    it('should fail when increaseApprovalCall', async function () {
      await token.pause()
      const message = await Message.new()
      ...
      await token.increaseApprovalAndCall(
        message.contract.address, 50, extraData, { from: accounts[0], value: 1000 }
      ).should.be.rejectedWith(EVMRevert)
    })

    it('should fail when decreaseApprovalCall', async function () {
      await token.pause()
      const message = await Message.new()
      ...
      await token.decreaseApprovalAndCall(
        message.contract.address, 50, extraData, { from: accounts[0], value: 1000 }
      ).should.be.rejectedWith(EVMRevert)
    })
  })
